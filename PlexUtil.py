...
def reconstruct_word_in_python(WORD_BITS,bits_num,unstrobed_bits,words_buffer,timestamps_buffer):
    ...
    oldest_timestamps = np.array([unstrobed_bits[bit][0] for bit in range(WORD_BITS)])
    # synonyms
    bits_num = bits_num
...
class PlexUtil(object):
    ...
    def GetExtEvents(self, data, event, bit=None, online=True):
        ...
        if event == 'unstrobed_word':
            ...
            unstrobed_bits_list = [timestamp[channel == bit+1] for bit in range(WORD_BITS)]
            bits_length = [len(unstrobed_bits_list[bit]) for bit in range(WORD_BITS)] # actural bits length
            ...
            unstrobed_bits = np.array([np.append(unstrobed_bits_list[bit], [infinity]*(max_length-bits_length[bit]+1)) \
                                       for bit in range(WORD_BITS)],dtype=np.float32)
